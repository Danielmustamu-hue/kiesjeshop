    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-24 animate-in fade-in duration-700">
        ...
        >
          ...
          <div className="absolute bottom-0 left-0 p-8 md:p-20 w-full md:w-3/4">
              <div className="flex items-center gap-4 mb-6">
                <span className="bg-orange-600 text-white px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest">{heroArticle.category}</span>
              ...
              <h3 className="text-3xl md:text-7xl font-black text-white mb-8 tracking-tighter group-hover:text-orange-400 transition-colors leading-[0.9]">{heroArticle.title}</h3>
              <div className="flex items-center gap-3 text-white font-black text-[11px] uppercase tracking-[0.4em] group-hover:gap-6 transition-all group-hover:text-orange-400">
                  Lees de volledige analyse <ArrowRight className="w-6 h-6" />
              </div>
          </div>
        </a>
      )}
        ...
        {subArticles.map((article) => (
          ...
          >
            ...
            <div className="p-10 flex flex-col flex-grow">
              <h3 className="text-2xl font-black text-slate-900 mb-6 group-hover:text-orange-600 transition-colors leading-tight tracking-tighter">{article.title}</h3>
              ...
              <div className="flex items-center justify-between pt-8 border-t border-slate-50 text-[10px] text-slate-400 font-black uppercase tracking-widest mt-auto">
                  ...
                  <div className="bg-slate-50 p-2.5 rounded-xl group-hover:bg-orange-600 group-hover:text-white transition-all">
      ...
      {!allLoaded && subArticles.length > 0 && (
        <div className="mt-24 text-center">
          <button onClick={() => setVisibleCount(v => v + 6)} className="inline-flex items-center gap-4 px-16 py-6 bg-slate-950 text-white font-black text-[11px] uppercase tracking-[0.4em] rounded-[2rem] hover:bg-orange-600 transition-all shadow-xl active:scale-95">
              Meer Artikelen Laden <ChevronDown className="w-5 h-5" />
          </button>
        </div>
      )}
    </div>
